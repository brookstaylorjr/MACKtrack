...
for img = 1:length(AuxImages)
    if ~isempty(AuxImages{img})
        ...
        for n = 1:nuc_cc.NumObjects
            CellMeasurements.(['MeanIntensity_nuc',num2str(img)])(n,iteration) = nanmean(img0(nuc_cc.PixelIdxList{n}));
            CellMeasurements.(['MedianIntensity_nuc',num2str(img)])(n,iteration) = nanmedian(img0(nuc_cc.PixelIdxList{n}));
            CellMeasurements.(['IntegratedIntensity_nuc',num2str(img)])(n,iteration) = nansum(img0(nuc_cc.PixelIdxList{n}));
        end
