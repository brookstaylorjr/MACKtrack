...
figure('position', positionfig(1048,644))
for itype=1:3
    ...
    for iname=1:length(cnames{itype,:})
        ...
        for icol=1:ncol
            ...
            text(-0.1, mean(Y), cnames{itype}{iname}, 'HorizontalAlignment', 'right', 'FontSize', 12, 'FontName', 'Arial','FontWeight','normal')
            xlim([-0.4, 1])
            hold all
        end % icol
        %set(gca, 'box', 'off')
        title(ctypes_title{itype}, 'FontSize', 16, 'FontName' , 'Arial')
        axis off
        set(gcf, 'color', [1 1 1])
    end % iname
    ylim([0.1 1.05.*max(Y)]);
